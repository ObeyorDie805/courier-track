...
newDestinationForm.addEventListener('submit', async (e) => {
  ...
  if (!/^\d{4}$/.test(code)) {
    alert('Please enter the 4-digit driver code.');
    return;
  }
  ...
  try {
    ...
    document.getElementById('driverCodeInput').value = code;
  } catch (err) {
    console.error(err);
...
document.getElementById('restroomBtn').addEventListener('click', () => {
  ...
  localStorage.setItem('route_' + tripId, JSON.stringify({ type: 'restroom', code }));
...
document.getElementById('stopBtn').addEventListener('click', () => {
  ...
  localStorage.setItem('route_' + tripId, JSON.stringify({ type: 'stop', code }));
